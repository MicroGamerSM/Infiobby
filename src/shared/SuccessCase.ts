...
export class SuccessCase {
	...
	static FromTuple(source: [boolean, string]) {
	...
	static Try<F extends (...args: readonly unknown[]) => void>(method: F, ...args: Parameters<F>): SuccessCase {
		try {
			...
			if (typeOf(exception) === "string") {
				return SuccessCase.Fail((exception as string) ?? "Failed to parse the error message as a string.");
			}
		}
...
export class ValueSuccessCase<T> {
	...
	static FromTuple<T>(source: [boolean, string, T]): ValueSuccessCase<T> {
	...
	static Try<F extends (...args: readonly unknown[]) => J, J>(
		...
		try {
			...
			return ValueSuccessCase.Fail(
				(exception as string) ?? "Failed to parse the error message as a string.",
				undefined,
			);
		}
	}
